...
for (const country of countries) {
  const option = `
    <li class="option d-flex flex-row align-items-center justify-content-between">
      <div>
        <img class="country-flag iconify" src="https://flagcdn.com/w320/${country.code.toLowerCase()}.png" alt="Flag">
      </div>
      <strong>+${country.phone}</strong>
    </li> `;

  select_box.querySelector('ol').insertAdjacentHTML('beforeend', option);
  options.push(select_box.querySelector('ol li:last-child'));
}
...
function searchCountry() {
  ...
  for (const option of options) {
    let is_matched = option.querySelector('.country-flag').alt.toLowerCase().includes(search_query);
